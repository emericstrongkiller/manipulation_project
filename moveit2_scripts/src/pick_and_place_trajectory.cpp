...
public:
  ...
  void execute_trajectory_plan() {
    RCLCPP_INFO(LOGGER,
                "Planning and Executing Approach Retreat Trajectory...");
    ...
    // setup the goal pose target
    RCLCPP_INFO(LOGGER, "Preparing Goal Pose Trajectory...");
    setup_goal_pose_target(+0.343, +0.132,
                           (target_pose_robot_.position.z - delta_), -1.000,
                           +0.000, +0.000, +0.000);
    // plan and execute the trajectory
    RCLCPP_INFO(LOGGER, "Planning Goal Pose Trajectory...");
    plan_trajectory_kinematics();
    ...
    RCLCPP_INFO(LOGGER, "Approach Retreat Trajectory Execution Complete");
  }
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> base_node =
      std::make_shared<rclcpp::Node>("approach_retreat_trajectory");

  // instantiate class
  ApproachRetreatTrajectory approach_retreat_trajectory_node(base_node);

  // execute trajectory plan
  approach_retreat_trajectory_node.execute_trajectory_plan();
